...
// Plugin for Mask Passwords plugin
import com.michelin.cio.hudson.plugins.maskpasswords.*

// Read properties
def home_dir = System.getenv("HOME")
def properties = new ConfigSlurper().parse(new File("$home_dir/data/jenkins.credentials").toURI().toURL())
...
properties.credentials.each {
  ...
  switch (it.value.type) {
    ...
    case "password":
      ...
      break
    case "secret":
      println "--> Create secret text credentials with token ${it.value.path}"
